...
def click_button_with_text(page: Page, texts: List[str], timeout: int = 2000) -> bool:
    # Try direct button/input/role=button
...
def accept_cookies(page: Page) -> None:
    ...
    # try a few times, silently
    for _ in range(3):
        if click_button_with_text(page, words, timeout=1000):
            return
        # common CMP buttons
        for sel in [
...
def random_category_click(page: Page, country: str) -> bool:
    """Pick a random category link on the homepage and open it."""
    # Collect category links on the start page
    try:
    ...
    for a in anchors:
        try:
            ...
            if not href.startswith("http"):
                continue
            # heuristics: category link looks like .../auto/ , .../mobil/ etc., not an ad
            if re.search(r"/inzerat/", href, flags=re.I):
                continue
            if re.match(rf"^https?://(www\.)?{re.escape(base)}/[a-z0-9\-]+/?$", href, flags=re.I):
    ...
    for url in candidates[:20]:
        try:
            ...
            accept_cookies(page)
            # Check if we landed on a category page that contains ads
            if page.query_selector("a[href*='/inzerat/']"):
                return True
            # Some categories paginate; still ok
            return True
        except Exception:
            continue
    return False
...
def open_reply_form(page: Page) -> bool:
    """Try to open contact/reply form on an ad page."""
    words = [
        "Odpovědět", "Odpovedať", "Reagovat", "Kontaktovat", "Kontaktovať",
        "Napísať", "Napsat", "Poslat zprávu", "Odoslať správu"
    ]
    if click_button_with_text(page, words, timeout=2000):
        time.sleep(0.5)
        return True
    # Fallback: anchor with those texts
    for w in words:
        try:
            page.locator(f"a:has-text('{w}')").first.click(timeout=1500)
            time.sleep(0.5)
            return True
        except Exception:
            pass
    ...
    **kwargs
    ...
    try:
        with sync_playwright() as pw:
            ...
            # context with profile hints (UA/locale/timezone/etc. — если заданы)
            context_args = {}
            if profile:
            ...
            if not random_category_click(page, country):
                print("[flow] No category found, staying on homepage.")
            else:
                accept_cookies(page)

            # 3) Pick random ad in that category
            if not random_ad_click(page):
                print("[flow] No ad link found on category page. Trying homepage fallback.")
            ...
            # 4) Open reply/contact form (if exists)
            open_reply_form(page)

            # 5) Fill phone, request code
            ...
            click_button_with_text(page, ["Získat kód","Získať kód","Odeslat kód","Poslať kód","Send code","Poslat kód"])

            # 6) Wait for code via provider
            code = None
            if get_code_fn:
                try:
            ...
            # 8) Heuristic success/error detection
            html = (page.content() or "").lower()
            success = any(x in html for x in ["ďakujem","dakujem","úspeš","úspěš","overen","ověřen","správa odoslaná","zpráva odeslána"])
            error = any(x in html for x in ["chybn","nespráv","neplat","znovu","error","chyba"])

            cookie_file = ""
            # save cookies by domain-country folder
            ...
            context.close()
            browser.close()
            return (success and not error), cookie_file
