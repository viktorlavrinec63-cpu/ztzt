...
def main():
    ...
    for i in range(1, runs+1):
        ...
        domain_cc = decide_domain_country(cfg, proxy_info)   # CZ or SK
        ...
        while cycles < max_number_cycles and not (success_cz and success_sk):
            ...
            if ((not success_cz and domain_cc == "CZ") or (not success_sk and domain_cc == "SK")):
                ok, file, reason = run_country_flow(domain_cc, phone, lambda timeout_sec=120: provider.wait_for_sms(act_id, timeout_sec), headful=args.headful, proxy=proxy_for_pw)
                ...
                if ok:
                    ...
                    if reason == "no_sms":
                        # Close this activation and get a NEW number for the SAME country
                        try:
                            provider.finalize(act_id, success=False, banned=banned)

                            finalized = True
                        except Exception as e:
                            print(f"[finalize] {e}")
                        try:
                            act_id, phone = provider.request_number(decide_sms_country(cfg, domain_cc))
                            print(f"[SMS {decide_sms_country(cfg, domain_cc)}] NEW number for {domain_cc}: {phone} (act_id={act_id})")
                        except Exception as e:
                            print(f"[{domain_cc}] failed to get new number: {e}")
                            break
                        # Start next cycle with new number
                        continue
                    else:
                        print(f"[{domain_cc}] will request a NEW number next…")
                        time.sleep(delay_between)
                        try: provider.request_new_code(act_id, domain_cc)
                        except Exception as e: print(f"[{domain_cc}] request_new_code: {e}")
            ...
            if (domain_cc == "CZ" and success_cz and not success_sk) or (domain_cc == "SK" and success_sk and not success_cz):
                ...
                ok, file, reason = run_country_flow(second, phone, lambda timeout_sec=120: provider.wait_for_sms(act_id, timeout_sec), headful=args.headful, proxy=proxy_for_pw)
                ...
                if ok:
                    ...
                    if reason == "no_sms":
                        ...
                        try:
                            ...
                            continue
                        ok2, file2, reason2 = run_country_flow(second, phone2,
                                                    lambda timeout_sec=120: provider.wait_for_sms(act2, timeout_sec),
                        ...
                        if ok2:
                            ...
                            okA, fileA, reasonA = run_country_flow(first_again, phone2,
                                                    lambda timeout_sec=120: provider.wait_for_sms(act2, timeout_sec),
                                                    headful=args.headful, proxy=proxy_for_pw)
                            print(f"[{first_again}] NEW-PAIR {('OK' if okA else 'FAIL')} reason={reasonA}; cookies: {fileA}")
                            if okA:
                                if first_again == 'CZ': success_cz = True
                                else: success_sk = True
                            # finalize the new activation considering both attempts
                            try:
